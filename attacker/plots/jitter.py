...
def generate_normalized_jitter_chart_mavlogdump(baseline_path, b_start_str, attack_path, a_start_str, duration_sec, output_image='normalized_jitter.png'):
    ...
    plt.plot(b_x, b_jitter, label='Baseline (Normalized)', color='#1f77b4', alpha=0.7, marker='^', markersize=6, linestyle='None')
    plt.plot(a_x, a_jitter, label='Attack (Normalized)', color='#d62728', alpha=0.8, marker='X', markersize=6, linestyle='None')
...
def generate_normalized_jitter_chart(baseline_tlog, b_start_str, attack_tlog, a_start_str, duration_sec, output_image='normalized_jitter.png'):
    """
    ...
    plt.figure(figsize=(12, 6))
    plt.plot(b_x, b_jitter, label='Baseline (Normalized)', color='#1f77b4', 
             alpha=0.7, marker='^', markersize=6, linestyle='None')
    plt.plot(a_x, a_jitter, label='Attack (Normalized)', color='#d62728', 
             alpha=0.8, marker='x', markersize=6, linestyle='None')

    plt.title(f'Normalized Telemetry Jitter ({duration_sec}s Window - Microsecond Accuracy)', fontsize=14)
    plt.xlabel('Seconds from Attack Start (t=0)', fontweight='bold')
    plt.ylabel('Inter-message Interval (ms)', fontweight='bold')
...
# Usage:
generate_normalized_jitter_chart('/home/wiktor/Desktop/Damn-Vulnerable-Drone/logs/baseline/comparison/tlogs/telemetry.tlog', "2026-04-15_21:11:13", '/home/wiktor/Desktop/Damn-Vulnerable-Drone/logs/socket_flood/2026-04-15_22:42:31/tlogs/telemetry.tlog', "2026-04-15_22:43:53", 60)